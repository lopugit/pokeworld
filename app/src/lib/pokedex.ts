// Gen I-III Pokédex data access + Gen III battle math shared by the
// encounter, battle, and UI layers. Data ships in src/data/pokedex.json
// (generated by scripts/fetch-pokedex.mjs).
...
const byName = new Map<string, PokedexEntry>(
  entries.map((entry) => [entry.name.toUpperCase(), entry]),
);
...
export const speciesByName = (name: string): PokedexEntry | undefined =>
  byName.get(name.trim().toUpperCase());

export function spriteUrl(
  id: number,
  options: { shiny?: boolean; back?: boolean } = {},
): string {
  const slot = options.back
    ? options.shiny
      ? "back-shiny/"
      : "back/"
    : options.shiny
      ? "shiny/"
      : "";
  return `/sprites/pokemon/gen3/${slot}${id}.png`;
}

// --- Gen III stat math (neutral nature, EVs ignored) ---

export interface StatBlock extends BaseStats {}
...
export const rollShiny = (rng: Rng, denominator = DEFAULT_SHINY_DENOMINATOR): boolean =>
  denominator > 0 && Math.floor(rng() * denominator) === 0;
...
export function typeEffectiveness(moveType: string, defenderTypes: string[]): number {
  ...
  return defenderTypes.reduce(
    (multiplier, type) => multiplier * (row[type.toUpperCase()] ?? 1),
    1,
  );
}
