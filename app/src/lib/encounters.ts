// Wild-encounter spawn rules: biome detection, geofencing, admin-editable
// per-species rule variants, and the encounter roll itself.
//
// Defaults are computed deterministically from the Pokédex (so the server
// only stores admin overrides): every species spawns world-wide, fenced by
// biome — grass-dwellers in long grass, water types while surfing, cave
// dwellers near cave mouths — at world levels 2-7. Legendaries ship with
// curated geofences (e.g. GROUDON within 100 km of Uluru at level 50-70).
// Admins may replace a species' rule list with any number of "variant"
// rules, each with its own fence, weight, level range, gender override and
// shiny odds.

import type { MapTile } from "../../server/services/map/types";
import {
  allSpecies,
  DEFAULT_SHINY_DENOMINATOR,
  getSpecies,
  rollGender,
  rollShiny,
  type PokedexEntry,
...
/** Admin override document: replaces ALL default rules for one species. */
export interface SpeciesSpawnOverride {
  speciesId: number;
  rules: SpawnRule[];
  updated?: number;
}
...
export const isWaterTile = (tile: MapTile | undefined): boolean =>
  !!tile &&
  (tile.terrain === "water" ||
    String(tile.img ?? "").startsWith("pond-") ||
    String(tile.img ?? "").startsWith("water"));

export const isLongGrassTile = (tile: MapTile | undefined): boolean =>
  !!tile && (tile.feature === "long-grass" || tile.img2 === "grass-2");
 ...
 * entrances (interiors are a future iteration), which `nearCave` reports.
 */
export function encounterBiomeFor(
  tile: MapTile | undefined,
  options: { surfing?: boolean; nearCave?: boolean } = {},
): EncounterBiome | null {
...
export const encounterTriggered = (biome: EncounterBiome, rng: Rng): boolean =>
  rng() < ENCOUNTER_STEP_CHANCE[biome];
...
const CAVE_SPECIES = new Set([
  35, 36, 41, 42, 46, 47, 50, 51, 63, 64, 65, 66, 67, 68, 74, 75, 76, 92, 93,
  94, 95, 104, 105, 132, 169, 202, 293, 294, 295, 296, 297, 302, 303, 304,
  305, 306, 307, 308, 337, 338, 343, 344, 353, 354, 355, 356, 360, 371, 372,
  373, 374, 375, 376,
]);

const WORLD_LEVEL_MIN = 2;
const WORLD_LEVEL_MAX = 7;
...
// Curated legendary geofences (all editable in the admin UI).
const LEGENDARY_HOMES: Record<number, LegendaryHome> = {
  // Kanto birds
...
export const defaultRuleId = (speciesId: number, index = 0): string =>
  index === 0 ? `${speciesId}:default` : `${speciesId}:default:${index}`;
...
export function defaultSpawnRules(): SpawnRule[] {
  return allSpecies().flatMap((entry) => defaultRulesFor(entry));
}

/** Applies admin overrides: an override replaces that species' entire rule list. */
export function mergeSpawnRules(overrides: SpeciesSpawnOverride[]): SpawnRule[] {
  const overridden = new Map(overrides.map((override) => [override.speciesId, override.rules]));
  return allSpecies().flatMap((entry) => {
    const custom = overridden.get(entry.id);
    return custom ?? defaultRulesFor(entry);
  });
}

// --- Rule validation (shared with the admin API) ----------------------------

const isValidFence = (fence: unknown): fence is GeoFence => {
  if (typeof fence !== "object" || fence === null) return false;
  const candidate = fence as Record<string, unknown>;
  if (candidate.kind === "global") return true;
  return (
    ...
    candidate.radiusKm <= 40075
  );
};

export function normalizeSpawnRule(value: unknown, speciesId: number, index: number): SpawnRule | null {
  if (typeof value !== "object" || value === null) return null;
  const rule = value as Record<string, unknown>;
  const biomes = Array.isArray(rule.biomes)
    ? rule.biomes.filter((biome): biome is EncounterBiome =>
        ENCOUNTER_BIOMES.includes(biome as EncounterBiome),
      )
    : [];
  if (!biomes.length || !isValidFence(rule.fence)) return null;
  const levelMin = Number(rule.levelMin);
  const levelMax = Number(rule.levelMax);
  if (!Number.isInteger(levelMin) || !Number.isInteger(levelMax)) return null;
  if (levelMin < 1 || levelMax > 100 || levelMin > levelMax) return null;
  const weight = Number(rule.weight);
  if (!Number.isFinite(weight) || weight < 0 || weight > 100000) return null;
  const shinyDenominator =
    rule.shinyDenominator === undefined || rule.shinyDenominator === null
      ? undefined
      : Number(rule.shinyDenominator);
  if (
    shinyDenominator !== undefined &&
    (!Number.isInteger(shinyDenominator) || shinyDenominator < 1 || shinyDenominator > 1_000_000)
  ) {
    return null;
  }
  const genderOverride =
    rule.genderOverride === "male" ||
    rule.genderOverride === "female" ||
    rule.genderOverride === "genderless"
      ? rule.genderOverride
      : undefined;
  return {
    id: typeof rule.id === "string" && rule.id ? rule.id : `${speciesId}:custom:${index}`,
    speciesId,
...
export function normalizeSpawnOverride(value: unknown): SpeciesSpawnOverride | null {
  if (typeof value !== "object" || value === null) return null;
  const record = value as Record<string, unknown>;
  const speciesId = Number(record.speciesId);
  if (!Number.isInteger(speciesId) || !getSpecies(speciesId)) return null;
  if (!Array.isArray(record.rules) || record.rules.length > 50) return null;
  const rules = record.rules
    .map((rule, index) => normalizeSpawnRule(rule, speciesId, index))
    .filter((rule): rule is SpawnRule => rule !== null);
  if (rules.length !== record.rules.length) return null;
  return { speciesId, rules, updated: Date.now() };
}
...
export function matchingRules(
  ...
  return rules.filter(
    (rule) =>
      rule.enabled &&
      rule.weight > 0 &&
      rule.biomes.includes(biome) &&
      fenceContains(rule.fence, lat, lng),
  );
}
...
export function rollEncounter(
  ...
  const level =
    chosen.levelMin + Math.floor(rng() * (chosen.levelMax - chosen.levelMin + 1));
  const gender =
