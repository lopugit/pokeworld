// @ts-nocheck -- faithful ESM port of the original map modifier.
...
const spawnChance = 0.9999
const spreadChance = 0.3
			...
			if (tile) {
				...
				if ((!tile.pondGenerated && priority > tilePriority) || state.regenerate) {
					...
					if (Object.values(colours).includes(tile.img2)) {
						...
						const invalidPondSibling = [sand, path, road]

						const tileSprite = functions.getTileSprite(tile.mapX, tile.mapY, state.tiles.cache)

						tile.needsSaving = true

						if (tileSprite === shortGrass) {
							if (
								!invalidPondSibling.includes(topMiddleSprite)
								&& !invalidPondSibling.includes(middleLeftSprite)
								&& !invalidPondSibling.includes(middleRightSprite)
								&& !invalidPondSibling.includes(bottomMiddleSprite)
								&& !invalidPondSibling.includes(topLeftSprite)
								&& !invalidPondSibling.includes(topRightSprite)
								&& !invalidPondSibling.includes(bottomLeftSprite)
								&& !invalidPondSibling.includes(bottomRightSprite)
							) {
								// maybe make pond
