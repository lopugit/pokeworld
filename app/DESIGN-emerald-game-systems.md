...
`localStorage["pokeworld:trainer:v1"]` — deliberately **not** in the
location-bound `things:v2` slices so it survives location changes.

```ts
interface TrainerState {
  version: 1;
  name: string;                       // default "LOPU"
  party: PartyMember[];               // starter: MUDKIP Lv.5
...
- **PartyPanel** — party list with ball icon, level, HP bar, types.
- **BagPanel** — pocket tabs (ITEMS / POKé BALLS / KEY ITEMS), quantities,
  description footer.
- **BadgesPanel** — badge case, 8 Hoenn badge slots, earned vs silhouette.
- **PcPanel** — item storage listing (withdraw/deposit: future iteration).
...
5. Server presets: villages (multi-house + signs + gardens), forest walls
   using unreferenced `big-tree-1..10`, hidden 1-wide carved paths ending in
   `field-item` tiles, ledge lines along contours, path width ≤ 3 + squared
   edges (these are landing via the parallel server-side agent).
6. Badge-earning events; PC withdraw/deposit; item use (POTION heals).
